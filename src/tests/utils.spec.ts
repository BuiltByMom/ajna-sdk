...
describe('Utility tests', () => {
  ...
  describe('add-account', () => {
    it('should add an account using the AddAccount class', () => {
      const provider = new providers.JsonRpcProvider(config.ETH_RPC_URL);
      const account = new AddAccount(provider);
      const KEY = '0x2bbf23876aee0b3acd1502986da13a0f714c143fcc8ede8e2821782d75033ad1';
      const wallet = account.addAccountFromKey(KEY);
      expect(wallet._isSigner).toBe(true);
      expect(wallet.address).toBe('0xbC33716Bb8Dc2943C0dFFdE1F0A1d2D66F33515E');
    });
  });
});
